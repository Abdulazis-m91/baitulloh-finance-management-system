...
export default function PengeluaranPesantren() {
  ...
  const nmBulanIni = bulanNames[cm];

  // ── Filter helper (sama persis dengan LaporanPesantren yang sudah benar) ──
  // Filter by tanggal transaksi
  const byTgl = (tgl: string) => {
    const d = new Date(tgl);
    return d.getMonth() === cm && d.getFullYear() === cy;
  };
  // Filter gabungan: by tanggal ATAU by kolom bulan (untuk handle deposit)
  ...
  const getSaldoDana = (dana: 'Konsumsi' | 'Operasional' | 'Pembangunan') => {
    const pendapatan = dana === 'Konsumsi'
      ? konsumsiData.filter(c => byTglAtauBulan(c.tanggal, c.bulan)).reduce((a, c) => a + c.nominal, 0)
        + pendapatanLainData.filter(p => byTgl(p.tanggal)).reduce((a, p) => a + p.nominal, 0)
      : dana === 'Operasional'
    ...
    const pengeluaran = pengeluaranList
      .filter(e => byTgl(e.tanggal) && e.jenis_keperluan.startsWith(dana))
  ...
  // ── Riwayat bulan ini ──
  const riwayatBulanIni = pengeluaranList.filter(e => byTgl(e.tanggal));
  const totalRiwayatPages = Math.max(1, Math.ceil(riwayatBulanIni.length / RIWAYAT_PAGE_SIZE));
  const pagedRiwayat = riwayatBulanIni.slice((riwayatPage - 1) * RIWAYAT_PAGE_SIZE, riwayatPage * RIWAYAT_PAGE_SIZE);

  // ── Rekap bulan ini ──
  const rekapBulanIni = (dana: string) => pengeluaranList.filter(e => byTgl(e.tanggal) && e.jenis_keperluan.startsWith(dana));
  ...
  const downloadNotaAsPDF = async (): Promise<boolean> => {
    ...
    try {
      ...
      return true;
    } catch { toast.error('Gagal mendownload nota'); return false; }
  };
  ...
  const doSave = () => {
    ...
    setShowNota(false); setKeterangan(''); setNominal(''); setJenisKeperluan(''); setRiwayatPage(1);
  ...
  const handleSimpanCetak = async () => {
    ...
    setIsSaving(false);
    if (downloaded) { doSave(); toast.success('Nota dicetak & data tersimpan'); }
  };

  const exportRekap = (dana: string) => {
    const data = rekapBulanIni(dana);
    const ws = XLSX.utils.json_to_sheet(data.map(d => ({ Tanggal: formatDate(d.tanggal), Keterangan: d.keterangan, Jenis: d.jenis_keperluan, Nominal: d.nominal, Petugas: d.petugas })));
  ...
  const tanggalNota = new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
  ...
  return (
    <div className="space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input value={keterangan} onChange={e => setKeterangan(e.target.value)} placeholder="Contoh: Pembelian bahan makanan"
              ...
              {/* Total Nominal Dana */}
              <div className={`flex items-center justify-between px-4 py-3 rounded-xl border text-sm ${saldoTersedia <= 0 ? 'bg-destructive/5 border-destructive/20' : sisaSetelahInput < 0 ? 'bg-warning/5 border-warning/20' : 'bg-success/5 border-success/20'}`}>
                <div>
                  <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mb-0.5">Total Nominal Dana {danaDigunakan}</p>
                  <p className={`font-extrabold text-lg ${saldoTersedia <= 0 ? 'text-destructive' : 'text-success'}`}>{formatRupiah(saldoTersedia)}</p>
                </div>
                {nominalInput > 0 && (
                  <div className="text-right">
                    <p className="text-xs text-muted-foreground font-semibold uppercase tracking-wider mb-0.5">Sisa Setelah Pengeluaran</p>
                    <p className={`font-extrabold text-lg ${sisaSetelahInput < 0 ? 'text-destructive' : 'text-primary'}`}>{formatRupiah(sisaSetelahInput)}</p>
                  </div>
                )}
              </div>
              {sisaSetelahInput < 0 && nominalInput > 0 && (
                <p className="text-xs text-warning font-semibold flex items-center gap-1">⚠️ Nominal melebihi saldo dana {danaDigunakan}</p>
              )}

              <div>
                <label className="text-xs font-semibold text-foreground mb-2 block uppercase tracking-wider">Nominal</label>
                <input value={nominal ? formatRupiah(parseInt(nominal)) : ''} onChange={e => setNominal(e.target.value.replace(/\D/g, ''))}
                  placeholder="Rp 0" className="w-full px-4 py-3.5 rounded-xl border border-border bg-background text-foreground placeholder:text-muted-foreground input-focus text-sm" />
              </div>
              ...
              <ol className="space-y-2">
                {tataTertib.map((t, i) => (
                  <li key={i} className="text-xs text-muted-foreground flex gap-2">
                    <span className="w-5 h-5 rounded-lg bg-primary/10 text-primary font-bold flex items-center justify-center flex-shrink-0 text-[10px]">{i + 1}</span>{t}
                  </li>
                ))}
              </ol>
            </div>
            ...
            <div className="p-5 space-y-3 min-h-[300px]">
              ...
              {pagedRiwayat.map((e, i) => (
                <motion.div key={e.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                  className="p-4 rounded-2xl bg-muted/30 border border-border group">
                  ...
                  <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
                    ...
                    <span>{e.jenis_keperluan.split(' - ')[1] || ''}</span>
                  </div>
                </motion.div>
              ))}
            </div>
            <div className="flex items-center justify-between px-5 py-4 border-t border-border bg-muted/10">
              <span className="text-xs text-muted-foreground">Halaman <span className="font-bold">{riwayatPage}</span> dari <span className="font-bold">{totalRiwayatPages}</span></span>
              <div className="flex gap-2">
                <button onClick={() => setRiwayatPage(p => Math.max(1, p - 1))} disabled={riwayatPage === 1} className="p-2 rounded-lg border border-border bg-background disabled:opacity-30 hover:bg-muted transition-colors"><ChevronLeft className="w-4 h-4" /></button>
                <button onClick={() => setRiwayatPage(p => Math.min(totalRiwayatPages, p + 1))} disabled={riwayatPage === totalRiwayatPages} className="p-2 rounded-lg border border-border bg-background disabled:opacity-30 hover:bg-muted transition-colors"><ChevronRight className="w-4 h-4" /></button>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      {(activeTab === 'rekap_konsumsi' || activeTab === 'rekap_operasional' || activeTab === 'rekap_pembangunan') && (() => {
        ...
        const data = rekapBulanIni(dana);
        const total = data.reduce((s, e) => s + e.nominal, 0);
        return (
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            <div className="flex justify-end">
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => exportRekap(dana)}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {data.length === 0 && (
                    <tr><td colSpan={6} className="py-16 text-center text-muted-foreground">
                      <div className="w-16 h-16 rounded-2xl bg-muted flex items-center justify-center mx-auto mb-3"><Receipt className="w-8 h-8 text-muted-foreground/30" /></div>
                      Belum ada data pengeluaran {dana} bulan ini
                    </td></tr>
                  )}
                  {data.map((e, i) => (
                ...
                <tfoot>
                  <tr className="bg-muted/50 border-t-2 border-border">
                    ...
                    <td className="py-4 px-4 text-xs text-muted-foreground font-semibold">{bulanNames[cm]} {cy}</td>
                    <td className="py-4 px-4 text-xs text-muted-foreground font-semibold">{data.length} Transaksi</td>
                    <td className="py-4 px-4 text-right font-extrabold text-destructive text-base">{formatRupiah(total)}</td>
            ...
            <motion.div initial={{ scale: 0.85, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
              ...
              <div ref={notaRef} className="bg-white rounded-xl overflow-hidden" style={{ fontFamily: 'Arial, sans-serif', fontSize: '11px' }}>
                <div style={{ background: '#1e40af', padding: '12px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  <div>
                    <div style={{ color: '#fff', fontWeight: 900, fontSize: '13px' }}>YAYASAN BAITULLOH</div>
                ...
                <div style={{ padding: '10px 14px' }}>
                  <div style={{ background: '#1e40af', padding: '4px 8px', borderRadius: '4px', marginBottom: '6px' }}>
                    <span style={{ color: '#fff', fontWeight: 700, fontSize: '9px' }}>DETAIL TRANSAKSI</span>
                  </div>
                  {[['No. Referensi', notaData.refNo], ['Tanggal', formatDate(notaData.tanggal)], ['Keterangan', notaData.keterangan], ['Dana Digunakan', notaData.dana_digunakan], ['Jenis Keperluan', notaData.jenis], ['Petugas', notaData.petugas]].map(([l, v]) => (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '10px' }}>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '9px', color: '#555' }}>Yukum Jaya, {tanggalNota}</div>
              ...
              <div className="grid grid-cols-3 gap-3 mt-5">
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleSimpanCetak} disabled={isSaving}
                ...
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={doSave}
                  ...
                  <Download className="w-5 h-5" /><span>Simpan</span>
                </motion.button>
                <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowNota(false)}
                  ...
                  <X className="w-5 h-5" /><span>Batal</span>
                </motion.button>
              </div>
            </motion.div>
          </motion.div>
