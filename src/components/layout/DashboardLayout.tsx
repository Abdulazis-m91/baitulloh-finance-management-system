...
export default function DashboardLayout() {
  ...
  const roleLabel =
    role === 'petugas_sekolah' ? 'Petugas Sekolah' :
    role === 'petugas_pesantren' ? 'Petugas Pesantren' :
    'Administrator';
  ...
  const SidebarContent = () => (
    <>
      ...
      <div className="p-6 border-b border-sidebar-border flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="lg:hidden ml-auto p-1.5 rounded-lg hover:bg-sidebar-accent text-sidebar-foreground/50"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
      ...
      {/* Navigation — scrollable jika menu banyak */}
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto mt-2">
              ...
              >
                {({ isActive }) => (
                  <>
                    ...
                    {isActive && (
                      <motion.div layoutId="sidebar-indicator-admin" className="ml-auto w-1.5 h-1.5 rounded-full bg-white"
        ...
        {(role === 'admin' || role === 'petugas_sekolah') && menuItemsSekolah.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
        ...
        {(role === 'admin' || role === 'petugas_pesantren') && menuItemsPesantren.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
      ...
      {/* User section — selalu di bawah, tidak ikut scroll */}
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* ── SIDEBAR DESKTOP — fixed permanen di kiri ── */}
      <aside className="hidden lg:flex flex-col fixed inset-y-0 left-0 w-72 gradient-sidebar z-40">
        <SidebarContent />
      </aside>
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="fixed inset-y-0 left-0 z-50 w-72 gradient-sidebar flex flex-col lg:hidden"
            >
              <SidebarContent />
            </motion.aside>
          </>
      ...
      {/* ── MAIN CONTENT — offset kiri sebesar lebar sidebar ── */}
